...
function sortBookByOrderAdd(books) {
  ...
  try {
    for(let i = 0 ; i < data.length ; i++) {
      ...
      //Add genres
      if(section[2].data["Gallery"].find(e =>  e.genres.name == data[i].genres.name) == undefined) section[2].data["Gallery"].push({genres : data[i].genres,count : 1})
      else {
        for(let i  = 0 ; i < section[2].data["Gallery"].length ; i++) {
          if(section[2].data["Gallery"][i].genres.name == data[i].genres.name ) section[2].data["Gallery"][i].count++;
        }
      }
    }
  } catch (error) {
