...
function RowItem(props) {
    switch (props.sectionName) {
        ...
        case 'Quotes':
            ...
            break;
        case 'Genres':
          const [activeGenresBox,setActiveGenresBox] = useState(null)
            return (
                <Row className="mb-4">
                    {props.data.map((e, i) => {
                        // if(e.addElement) {
                        ...
                        // }
                        if (!e.addElement) {
                            return <GenresItem data={e} key={i} numbercolumn={props.numbercolumn} 
                                                handleClickGenresElement = {(genres) => setActiveGenresBox(genres)}
                                    />;
                        }
                    })}
                    {
                      activeGenresBox && 
                      <GenresBox
                        name ={activeGenresBox}
                        handleCloseGenresBox = {() => setActiveGenresBox(null)}
                      ></GenresBox>
                    }
                </Row>
            );
...
function GenresItem(props) {
    return (
        <Col lg={12 / props.numbercolumn} onClick={() => props.handleClickGenresElement(props.data.genres.name)}>
            <ListGroup>
                <ListGroup.Item className={`${style.content} ${style.content__genres}`}>
                    <p className={style.content__genres__title}>
                        <cite>
                            {<i className={props.data.genres.icon}></i>}
                            {props.data.genres.name}
                        </cite>
                    </p>
...
function AddBlock(props) {
    let addBlockType =
        props.type == 'Books'
            ? style.booksAddBlock
            : props.type == 'Genres'
            ? style.genresAddBlock
            : style.quotesAddBlock;
    ...
    return props.pos == 'newcolumn' ? (
        <Col lg={12 / props.numbercolumn} onClick={() => console.log(props.type)}>
            <ListGroup>
                <ListGroup.Item
                    className={`${style.content} ${style.newcol} ${addBlockType} `}
                    onClick={() => setShowBookBox(true)}
                >
                    <h4>+ New</h4>
                </ListGroup.Item>
            </ListGroup>
